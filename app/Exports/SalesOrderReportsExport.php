...
class SalesOrderReportsExport implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents
{
    protected $from_date;
    protected $to_date;
    ...
    public function collection()
    {
        $data = DB::table('sales_orders')
            ...
            ->when(!empty($this->from_date), function ($query) {
                $query->where('sales_orders.created_at', '>=', $this->from_date . ' 00:00:00');
            })
            ->when(!empty($this->to_date), function ($query) {
                $query->where('sales_orders.created_at', '<=', $this->to_date . ' 23:59:59');
            })
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                // 设置标题行字体大小和加粗
                $cellRange = 'A1:O1'; // 根据列数调整范围，这里是15列
