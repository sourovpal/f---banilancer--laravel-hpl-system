...
@if (!empty($configData['mainLayoutType']) && isset($configData['mainLayoutType']))
    @include(
        $configData['mainLayoutType'] === 'horizontal-menu'
...
<script>
    if ('{{ Auth::user()->role }}' == 'internal') {
        if ('{{ Auth::user()->admin_role }}' != 0) {
            ...
            $('.administration_menu').parent().find('.collapsible-sub li:nth-child(3)').hide();
        }
        $('.quotation_menu').parent().find('.collapsible-sub li:nth-child(2)').hide();
    }

    if ('{{ Auth::user()->role }}' == 'external') {
        $('.quotation_menu').parent().show()
        $('.quotation_menu').parent().find('.collapsible-sub li:nth-child(3)').hide();
    }
</script>
