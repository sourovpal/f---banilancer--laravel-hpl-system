...
{{-- page title --}}
@section('title','Create Good Receive')
...
<section class="users-list-wrapper section">
  <div class="users-list-table">
    <form id="goodreceiveRegisterForm" action="/goodreceive_register" method="post">
      {{ csrf_field() }}
            ...
            <div class="col s12 m6">
              ...
              <select id="supplier" name="supplier">
                ...
                @foreach($suppliers as $supplier)
                  <option value="{{ $supplier -> id }}" {{ $supplier -> id == $selectedSupplier ? 'selected' : '' }}>{{ $supplier -> englishname }}</option>
                @endforeach
              </select>
            </div>
          </div>
        ...
        <div class="card-content">
          ...
          <div class="responsive-table mt-1">
            <table id="item_table" class="users-list-datatable table">
              ...
              @foreach ($purchaseorderitems as $gritem)
                @if ($gritem['qty'])
                  <tr class="item_row">
                    ...
                    <td>{{ $gritem['pack'] }}</td>
                    <td class="item_cost">
						<input type="text" name="items[{{ $gritem['id'] }}][cost]" class="table_input_cost" value="{{ $gritem['price'] }}">
					</td>
					<td class="item_qty">
						<input type="text" name="items[{{ $gritem['id'] }}][qty]" class="table_input" value="{{ $gritem['qty'] }}">
					</td>
